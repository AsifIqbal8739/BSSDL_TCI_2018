function [Iksvd,Ibksvd,output] = denoiseImageKSVD(Image,sigma,K,varargin)
%==========================================================================
...
%% train dictionaries using ksvd and bksvd

d0 = 1:K; 
max_it = 5;
s = 3;
k = 3;
[D1,D2,d2] = compare_ksvd(blkMatrix,K,k,s,d0,max_it);
save denoise;
...
%% denoise the image using the resulted dictionary of KSVD
Dictionary = D1;
...
%     end
    ...
    %Coefs_omp = OMPerr(Dictionary,blocks(:,jj:jumpSize),errT);
    [X1 Coefs] = simult_sparse_coding(D1,blocks(:,jj:jumpSize),d0,k*s,0);
    if (reduceDC)
        blocks(:,jj:jumpSize)= Dictionary*X1 + ones(size(blocks,1),1) * vecOfMeans;
    else
        blocks(:,jj:jumpSize)= Dictionary*X1 ;
    end
end
...
%    close(h);
%end
Iksvd = (Image+0.034*sigma*IMout)./(1+0.034*sigma*Weight);
...
%     Coefs = OMPerr(Dictionary,blocks(:,jj:jumpSize),errT);
    [X2 Coefs] = simult_sparse_coding(D2,blocks(:,jj:jumpSize),d2,k,0);
    if (reduceDC)
        blocks(:,jj:jumpSize)= Dictionary*X2 + ones(size(blocks,1),1) * vecOfMeans;
    else
        blocks(:,jj:jumpSize)= Dictionary*X2 ;
    end
end
...
%     close(h);
% end
Ibksvd = (Image+0.034*sigma*IMout)./(1+0.034*sigma*Weight);
